...
@locacoes_router.post(
    "/locacoes/", response_model=Locacao, status_code=HTTPStatus.CREATED
)
def criar_locacao(locacao: Locacao):
    ...
    append_csv(file, campos, locacao.model_dump())
    return locacao
...
def atualizar_locacao(id: uuid.UUID, atualizado: Locacao):
    for index, l in enumerate(locacoes):
        if l.id == id:
            ...
            write_csv(file, campos, [locacao.model_dump() for locacao in locacoes])
            return atualizado
    raise HTTPException(
        status_code=HTTPStatus.NOT_FOUND, detail="Locação não encontrada."
    )
